...
Breadcrumbs::register('home', function($breadcrumbs) {
    $breadcrumbs->push(Lang::get('menu.home'), route('home'));
});
...
Breadcrumbs::register('test.add', function($breadcrumbs, $courseId, $sectionId) {
    $breadcrumbs->parent('courses.show', $courseId);
    $breadcrumbs->push(Section::whereId($sectionId)->first()->name, route('courses.show', $courseId));
    $breadcrumbs->push('Add test', route('test.add', $courseId, $sectionId));
});

Breadcrumbs::register('test.show', function($breadcrumbs, $courseId, $sectionId, $testId) {
    $breadcrumbs->parent('courses.show', $courseId);
    $breadcrumbs->push(Section::whereId($sectionId)->first()->name, route('courses.show', $courseId));
    $breadcrumbs->push('Test:' . Test::whereId($testId)->first()->name, route('test.show', $courseId, $sectionId, $testId));
});

Breadcrumbs::register('test.showAttemptResult', function($breadcrumbs, $courseId, $sectionId, $testId,$attempt) {
    $breadcrumbs->parent('courses.show', $courseId);
    $breadcrumbs->push(Section::whereId($sectionId)->first()->name, route('courses.show', $courseId));
    $breadcrumbs->push('Test:' . Test::whereId($testId)->first()->name, route('test.showAttemptResult', $courseId, $sectionId, $testId,$attempt));
});

Breadcrumbs::register('test.edit', function($breadcrumbs, $courseId, $sectionId, $testId) {
    $breadcrumbs->parent('courses.show', $courseId);
    $breadcrumbs->push(Section::whereId($sectionId)->first()->name, route('courses.show', $courseId));
    $breadcrumbs->push('Test:' . Test::whereId($testId)->first()->name, route('test.show', $courseId, $sectionId, $testId));
    $breadcrumbs->push('Edit', route('test.edit', $courseId, $sectionId, $testId));
});

Breadcrumbs::register('test.addQuestion', function($breadcrumbs, $courseId, $sectionId, $testId) {
    $breadcrumbs->parent('test.show', $courseId, $sectionId, $testId);
    $breadcrumbs->push('Add question', route('test.addQuestion', $courseId, $sectionId, $testId));
});
...
Breadcrumbs::register('assignment.submissions', function($breadcrumbs, $courseId, $sectionId, $assignmentId) {
    $breadcrumbs->parent('assignment.show', $courseId, $sectionId, $assignmentId);
    $breadcrumbs->push('Submissions', route('assignment.submissions', $courseId, $sectionId, $assignmentId));
});
...
Breadcrumbs::register('admincp', function($breadcrumbs) {
    $breadcrumbs->push('Admin Control Panel', route('admincp'));
});

Breadcrumbs::register('admincp.reports', function($breadcrumbs) {
    $breadcrumbs->parent('admincp');
    $breadcrumbs->push('Reports', route('admincp.reports'));
});

Breadcrumbs::register('admincp.config', function($breadcrumbs) {
    $breadcrumbs->parent('admincp');
    $breadcrumbs->push('Site Configuration', route('admincp.config'));
});

Breadcrumbs::register('admincp.categories', function($breadcrumbs) {
    $breadcrumbs->parent('admincp');
    $breadcrumbs->push('Course Categories', route('admincp.categories'));
});

Breadcrumbs::register('admincp.categories.create', function($breadcrumbs) {
    $breadcrumbs->parent('admincp.categories');
    $breadcrumbs->push('Add new category', route('admincp.categories.create'));
});

Breadcrumbs::register('admincp.categories.edit', function($breadcrumbs) {
    $breadcrumbs->parent('admincp.categories');
    $breadcrumbs->push('Edit Category', route('admincp.categories.edit'));
});